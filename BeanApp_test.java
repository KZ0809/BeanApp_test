...
public class BeanApp_test extends JFrame { // クラス名を確認
    //DBへの接続環境作成
    ...
    public BeanApp_test() {
        ...
        try {
            ...
            try (Statement stmt = conn.createStatement()) {
                ...
                stmt.execute("CREATE TABLE IF NOT EXISTS beans (" +
                             ...
                             "temperature DOUBLE PRECISION, " + // 数値型、NULLを許容
                             "humidity DOUBLE PRECISION, " +   // 数値型、NULLを許容
                             "input_weight DOUBLE PRECISION, " + // 数値型、NULLを許容
                             "output_weight DOUBLE PRECISION, " + // 数値型、NULLを許容
                             "roast_Index TEXT, " +    // NULLを許容
                             "roast_Degree TEXT, " +   // NULLを許容
                             "bottom DOUBLE PRECISION)");          // 数値型、NULLを許容
        ...
        roastScrollPane.setPreferredSize(new Dimension(350, (int)(500 * 0.4))); 
        ...
        chartPanel.setPreferredSize(new Dimension(350, (int)(500 * 0.4)));
        ...
        if (roastPanel != null && roastPanel.tableModel != null) {
            ...
            for (int i = 0; i < model.getRowCount(); i++) {//焙煎表の２行目である温度を取得
                ...
                if (tempValueObject != null) {//nullでないかの確認
                    // parseDoubleSafelyでNaNを返すため、直接使う
                    double currentTemp = model.parseDoubleSafely(tempValueObject);

                    // NaNや無限大、そして0.0は最低値として扱わない
                    if (!Double.isNaN(currentTemp) && !Double.isInfinite(currentTemp) && currentTemp != 0.0) {
    ...
    private boolean saveBeanInternal() {
        try {
            ...
            Double temperature; // Doubleオブジェクトに変更
            try {//気温が数値かどうかを判定する。数値でないならエラー
                temperature = tempField.getText().trim().isEmpty() ? Double.NaN : Double.parseDouble(tempField.getText().trim());
            } catch (NumberFormatException e) {
                JOptionPane.showMessageDialog(this, "気温には数値を入力してください。", "入力エラー", JOptionPane.ERROR_MESSAGE);
                return false;
            }

            Double humidity; // Doubleオブジェクトに変更
            try {//湿度が数値かどうかを判定する。数値でないならエラー
                humidity = humidField.getText().trim().isEmpty() ? Double.NaN : Double.parseDouble(humidField.getText().trim());
            } catch (NumberFormatException e) {
                JOptionPane.showMessageDialog(this, "湿度には数値を入力してください。", "入力エラー", JOptionPane.ERROR_MESSAGE);
                return false;
            }
            
            Double inputWeight; // Doubleオブジェクトに変更
            try {//投入温度が数値かどうかを判定する。数値でないならエラー
                inputWeight = inputField.getText().trim().isEmpty() ? Double.NaN : Double.parseDouble(inputField.getText().trim());
            } catch (NumberFormatException e) {
                JOptionPane.showMessageDialog(this, "投入量には数値を入力してください。", "入力エラー", JOptionPane.ERROR_MESSAGE);
                return false;
            }

            Double outputWeight; // Doubleオブジェクトに変更
            try {//引き上げ量が数値かどうかを判定する。数値でないならエラー
                outputWeight = outputField.getText().trim().isEmpty() ? Double.NaN : Double.parseDouble(outputField.getText().trim());
            } catch (NumberFormatException e) {
                JOptionPane.showMessageDialog(this, "引き上げ量には数値を入力してください。", "入力エラー", JOptionPane.ERROR_MESSAGE);
                return false;
            }

            Double bottomValue; // Doubleオブジェクトに変更
            try {//ボトムが数値かどうかを判定する。数値でないならエラー
                bottomValue = bottomField.getText().trim().isEmpty() ? Double.NaN : Double.parseDouble(bottomField.getText().trim());
            } catch (NumberFormatException e) {
                 JOptionPane.showMessageDialog(this, "ボトムには数値を入力してください。", "入力エラー", JOptionPane.ERROR_MESSAGE);
                 return false;
            }

            double index = caluculateIndex(inputWeight.isNaN() ? 0.0 : inputWeight, outputWeight.isNaN() ? 0.0 : outputWeight);//焙煎指数を計算 (NaNの場合は0.0として計算)
            ...
            if (isUpdateMode) {
                ...
                // Double.isNaN() でチェックし、NaNの場合はNULLをセット
                if (temperature.isNaN()) ps.setNull(5, Types.DOUBLE); else ps.setDouble(5, temperature);
                if (humidity.isNaN()) ps.setNull(6, Types.DOUBLE); else ps.setDouble(6, humidity);
                if (inputWeight.isNaN()) ps.setNull(7, Types.DOUBLE); else ps.setDouble(7, inputWeight);
                if (outputWeight.isNaN()) ps.setNull(8, Types.DOUBLE); else ps.setDouble(8, outputWeight);
                
                ps.setString(9, roastIndex);
                ps.setString(10, roastDegree);
                if (bottomValue.isNaN()) ps.setNull(11, Types.DOUBLE); else ps.setDouble(11, bottomValue);
                ...
                // Double.isNaN() でチェックし、NaNの場合はNULLをセット
                if (temperature.isNaN()) ps.setNull(6, Types.DOUBLE); else ps.setDouble(6, temperature);
                if (humidity.isNaN()) ps.setNull(7, Types.DOUBLE); else ps.setDouble(7, humidity);
                if (inputWeight.isNaN()) ps.setNull(8, Types.DOUBLE); else ps.setDouble(8, inputWeight);
                if (outputWeight.isNaN()) ps.setNull(9, Types.DOUBLE); else ps.setDouble(9, outputWeight);
                
                ps.setString(10, roastIndex);
                ps.setString(11, roastDegree); 
                if (bottomValue.isNaN()) ps.setNull(12, Types.DOUBLE); else ps.setDouble(12, bottomValue); 
    ...
    private void showViewPanel(String beanName) {
        try {
            ...
            while (rsRoast.next()) {//焙煎表の表示
                ...
                // グラフデータに追加 (DBがNULLでなかった、かつ有効な数値の場合のみプロット)
                // 0.0の値もプロット対象に含める（NaNやInfiniteは除外）
        ...
        private void loadRoastDataForBean(String beanName) {
            ...
            try {
                ...
                while (rs.next()) {
                    ...
                    boolean tempWasNull = rs.wasNull(); // temperatureがDBでNULLだったか判定
                    double tempChange = rs.getDouble("temp_change");
                    boolean tempChangeWasNull = rs.wasNull(); // temp_changeがDBでNULLだったか判定
                    String gasPressure = rs.getString("gas_pressure");
                    String damper = rs.getString("damper");
                    String note = rs.getString("note");
                    
                    // DBのNULL値はNaNとしてロード
                    loadedData.add(new Object[]{
                        time, 
                        tempWasNull ? Double.NaN : temp, 
                        tempChangeWasNull ? Double.NaN : tempChange, 
                        gasPressure, 
                        damper, 
                        note
                    });
    ...
    class RoastTableModel extends AbstractTableModel {
        ...
        public boolean isCellEditable(int rowIndex, int columnIndex) {
            // 温度変化の列（インデックス2）は自動計算なので編集不可にするのが一般的
            if (columnIndex == 2) {
                return false; // 温度変化は編集不可
            }
            // 時間の列（インデックス0）も固定なので編集不可にするのが一般的
            if (columnIndex == 0) {
                return false; // 時間は編集不可
            }
